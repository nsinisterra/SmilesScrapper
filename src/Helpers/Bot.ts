...
export default class Bot {
  ...
  async start() {
    ...
    this._bot.start((ctx) => {
      ctx.reply('Welcome! This is a private bot. Please, leave me, I want to be alone.');        
    });

    this._bot.on('text', (ctx) => {
      console.log(`Message received from ${ctx.message.from.id} | ${ctx.message.from.username}: ${JSON.stringify(ctx.message)} `)
    });
  ...
  parseResultToText(result:IResult): string {
      ...
      if(result.businessResults > 0){
        text += `\r\n Best Business Flight: (${result.bestBusinessFlight.date.toLocaleString(config.locale)}) | Smiles: ${result.bestBusinessFlight.smiles.toLocaleString(config.locale)} | Paradas: ${result.bestBusinessFlight.stops} | Duración: ${result.bestBusinessFlight.duration}`;
      }      
